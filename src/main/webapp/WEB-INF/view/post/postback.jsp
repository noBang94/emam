...
<%--            <%}%>--%>
                ...
                $(this).parents(".rp-r-w-btn-warp").append(`
                    <span id="spn\${reindex}">
                        <form action="<%=request.getContextPath() %>/reply/replyInsert.do" method="post">
                            ...
                            <button type="submit" class="rereply-insert-btn" data-reindex="\${reindex}">답글달기</button>
                        </form>
                    </span>
                `);
            ...
            function loadData() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#output-container').text(result.trim());
                        setseiper();
                        page++; // 페이지 번호 증가
                        loading = false;
                    },
    ...
    <style>
        *{padding: 0;margin: 0;}
        ...
        /*공통으로 사용할만한것(s)*/
        .modal{display: none;width: 100%; height: 100%; position: fixed; top: 0; left: 0; background: rgba(0, 0, 0, 0.5); z-index: 3;}
        ...
        .post-ipt{width: 100%;display: block;height: 100%;background: green;opacity: 100%;overflow: hidden;
            position: absolute;
            top: 0;
            left: 0;}
        .modal-body{display: flex;width: 100%;}
        .modal-l{position: relative; width: 100%;}
        .modal-r{
            width: 0;
            overflow: hidden;
        }
